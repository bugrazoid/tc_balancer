...
/// This crate has a runtime dependency on artifacts produced by the `tc_balancer-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "tc_balancer-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `tc_balancer-ebpf`, so cargo will automatically rebuild us
        // if `tc_balancer-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `tc_balancer-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("tc_balancer-ebpf");
